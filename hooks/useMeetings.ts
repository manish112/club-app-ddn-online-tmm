  ...
  const fetchAll = useCallback(async () => {
    const [{ data: meetingsData }, { data: membersData }, { data: ballotsData }, { data: annData }] = await Promise.all([
      ...
      supabase.from('members')
        .select('id, membership_no, name, display_name, active, is_admin, can_manage_guests, introduction, mentor_id, leadership_roles, phone, email, city, gender, avatar_url, show_phone_in_contact, theme_preference, created_at, email_consent_status, email_consent_at, email_consent_device, whatsapp_consent_status, whatsapp_consent_at, whatsapp_consent_device, terms_accepted_version, terms_accepted_at, terms_accepted_device')
