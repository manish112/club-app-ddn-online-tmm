  ...
  const fetchAll = useCallback(async () => {
    const [{ data: meetingsData }, { data: membersData }, { data: ballotsData }, { data: annData }] = await Promise.all([
      ...
      supabase.from('members')
        .select('id, membership_no, name, display_name, active, is_admin, introduction, mentor_id, leadership_role, phone, email, city, gender, avatar_url, show_phone_in_contact, created_at')
